   ...
   {      
      ...
      for (MenuItem item : menu.getItems()) {
         menuOut.append(String.format("{label: \"%s\", imageUrl: \"%s\", href: \"%s\"},", 
               item.getLabel(), item.getImageUrl(), item.getHref()));
      }
	...
	/*public static <E> HtmlAndJavaScript getGridForObjects(Collection<E> objects, Class<E> classObj, String cssClass)
	{
	   String randomStr = RandomStringUtils.random(12, true, true);
		String gridId = "gcs_grid_" + randomStr;
		String pagerId = "gcs_grid_pager_" + randomStr;
		
		StringBuilder htmlOut = new StringBuilder();
		htmlOut.append(String.format("<table id=\"%s\" class=\"jq-grid %s\">", gridId, cssClass));
		htmlOut.append(String.format("</table>"));
		htmlOut.append(String.format("<div id=\"%s\" class=\"jq-grid-pager\">", pagerId));
		htmlOut.append(String.format("</div>"));
		...
		jsOut.append(String.format("  $(\"#%s\").jqGrid(\"navGrid\", \"#%s\", {", gridId, pagerId));
		jsOut.append(String.format("    edit: false,"));
