...
public class PrivateResourceFilter implements Filter
{
    ...
    * 
    * @param contextPath
    *           Context path because the uris we have do not have the context
    * @param resourceUri
    *           The resource uri to test
   ...
   public void init(FilterConfig config) throws ServletException
   {
      ...
      for (String publicResourceUrl : splitPublicResources) {
         // Generate the regex
         String publicResourceUrlRegex = "^" + (contextPath + publicResourceUrl).replace(".", "\\.")
                     .replace("*", ".*").replace("/", "\\/") + "$";

         // Compile the regex as a predicate
         publicResourceUriPredicates.add(Pattern
               .compile(publicResourceUrlRegex).asPredicate());
      }
   }
}
