...
{
    /**
     * The hash map for all module's roles rights The first map sorts them by
     * module; The second map sorts them by roles.
     */
    private Map<String, Map<String, Rights>> moduleRights = new CaseInsensitiveMap<>();
    ...
    /**
     * Returns whether a certain role can or cannot do a certain operation on a
     * certain module
     * 
     * @param moduleName The module name to test
     * @param roleName The role name to test
     * @param operation The CRUD operation
     * @return whether a certain role can or cannot do a certain operation on a
     *         certain module
     */
    @Override
    ...
    private void loadModuleSecurity(String moduleSecurityFolderAbsolutePath) throws JDOMException, IOException
    {
        ...
        Collection<Element> rootElements = XmlUtils.loadXmlFiles(new RegexFilenameFilter(appMessagesRegex),
                moduleSecurityFolderAbsolutePath);
        ...
        for (Element root : rootElements) {
            ...
            for (Element module : moduleElements) {
                ...
                if (moduleRights.containsKey(moduleName)) {
                    ...
                    rightsMap = new CaseInsensitiveMap<>();
                ...
                for (Element role : roleElements) {
                    ...
                    if (!rightsMap.containsKey(roleName)) {
                        ...
                        Collection<CrudOperation> rightsOp = new ArrayList<>();
